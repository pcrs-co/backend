...
def find_consensus_response(responses: list[str]):
    ...
    valid_data = [
        ...
        if d and "requirements" in d and len(d["requirements"]) == 2
    ]

    if not valid_data:
        raise ValueError(
            "None of the AI responses could be parsed into a valid structure."
        )
    ...
    for data in valid_data:
        ...
        if rec_dict:
            ...
            if spec_tuple == most_common_spec:
                print(
                    f"Consensus found. Choosing response with recommended specs: {spec_tuple}"
                )
